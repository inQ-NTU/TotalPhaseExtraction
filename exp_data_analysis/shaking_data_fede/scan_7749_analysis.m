...
load('scan7749_exp_data_for_phase_extraction_all.mat')
...
n_max_fourier = 20;
idx_begin = 28; %The start of untruncated index
idx_end = 72; %The end of untruncated index
...
for j = 1:length(evol_time)
    ...
    for i = 1:num_samples
        com_ext_suite = class_common_phase_extraction(density_ripple_cut(:,i), mean_density_cut, z_cut, t_tof, 1);
...
[fitted_curve,gof] = fit(z_cut',g1_midcut',fitfun);
...
plot(evol_time, avg_cosineCoeffs_all(:,1), 'o', 'Color', 'red', 'LineWidth',1.1, 'MarkerSize', 4)
hold on
